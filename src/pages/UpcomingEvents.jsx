...
export default function UpcomingEvents() {
  ...
  const [inscribedIds, setInscribedIds] = useState([]);
  ...
  const load = async () => {
    ...
    try {
      ...
      const activeInscribedIds = inscriptionsRes.data.map((i) => i.event.id);
      setInscribedIds(activeInscribedIds);
        ...
        {events.map((event) => {
          ...
          return (
            <div key={event.id} style={s.card}>
              ...
              {isInscribed ? (
                <button style={s.btnInscribed} disabled>
                  Ya estás inscrito
                </button>
              ) : (
                <button onClick={() => inscribe(event.id)} style={s.btn}>
                  Inscribirse →
                </button>
              )}
            </div>
